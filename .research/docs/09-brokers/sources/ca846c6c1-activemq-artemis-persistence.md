...
- [1\. File Journal (Default)](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#file-journal-default)
  - [1.1. Java NIO](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#java-nio)
  - [1.2. Linux Asynchronous IO](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#linux-asynchronous-io)
  - [1.3. Memory Mapped](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#memory-mapped)
  - [1.4. Journal and Data Retention](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#journal-and-data-retention)
  - [1.5. Standard Files](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#standard-files)
  - [1.6. Installing AIO](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#installing-aio)
- [2\. JDBC Persistence](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#jdbc-persistence)
  - [2.1. Configuring JDBC Persistence](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#configuring-jdbc-persistence)
  - [2.2. Configuring JDBC connection pooling](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#configuring-jdbc-connection-pooling)
- [3\. Zero Persistence](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#zero-persistence)

Apache Artemis ships with two persistence options.
...
Apache Artemis ships with three implementations - NIO, AIO, & Memory Mapped.

### [1.1. Java NIO](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html\#java-nio)

The first implementation uses standard Java [NIO](https://en.wikipedia.org/wiki/New_I/O) to interface with the file system.
...
Instructions on how to install libaio are referenced [here](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#installing-aio).

Also, please note that AIO will only work with the following file systems: ext2, ext3, ext4, jfs, xfs, and NFSv4.

For more information on libaio please see [lib AIO](https://artemis.apache.org/components/artemis/documentation/latest/libaio.html#libaio-native-libraries).

libaio is part of the Linux kernel project.

### [1.3. Memory Mapped](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html\#memory-mapped)

The third implementation uses a file-backed [READ\_WRITE](https://docs.oracle.com/javase/8/docs/api/java/nio/channels/FileChannel.MapMode.html#READ_WRITE) [memory mapping](https://en.wikipedia.org/wiki/Memory-mapped_file) against the OS page cache to interface with the file system.
...
Under power failure durability requirements, it will perform at least on par with the NIO journal with the only exception of Linux OS with kernel less or equals 2.6, in which the [_msync_](https://docs.oracle.com/javase/8/docs/api/java/nio/MappedByteBuffer.html#force%28%29)) implementation necessary to ensure durable writes was different (and slower) from the [_fsync_](https://docs.oracle.com/javase/8/docs/api/java/nio/channels/FileChannel.html#force%28boolean%29) used is case of NIO journal.

It benefits by the configuration of OS [huge pages](https://en.wikipedia.org/wiki/Page_%28computer_memory%29), in particular when is used a big number of journal files and sizing them as multiple of the OS page size in bytes.

### [1.4. Journal and Data Retention](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html\#journal-and-data-retention)

Historical data can be stored and replayed.
Refer to [Data Retention](https://artemis.apache.org/components/artemis/documentation/latest/data-retention.html) chapter for more information.
...
This is discussed in [Large Messages](https://artemis.apache.org/components/artemis/documentation/latest/large-messages.html#large-messages).

The broker can also be configured to page messages to disk in low-memory situations.
This is discussed in [Paging](https://artemis.apache.org/components/artemis/documentation/latest/paging.html#paging).

If no persistence is required at all, that can also be configured as discussed [here](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html#zero-persistence).

#### [1.5.1. Configuring the bindings journal](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html\#configuring-the-bindings-journal)
...
#### [1.5.2. Configuring the jms journal](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html\#configuring-the-jms-journal)

The jms config shares its configuration with the bindings journal.

#### [1.5.3. Configuring the message journal](https://artemis.apache.org/components/artemis/documentation/latest/persistence.html\#configuring-the-message-journal)
...
This is useful when the message journal is on a SAN and is being used a [Shared Store HA](https://artemis.apache.org/components/artemis/documentation/latest/ha.html#shared-store) policy with the broker instances on the same physical machine.
...
Notice: in case you get too many files you can use [compacting](https://artemis.apache.org/components/artemis/documentation/latest/data-tools.html#data-tools).
...
If you are running Linux kernel 2.6 or later and don’t already have `libaio` installed you can easily install it using the [following steps](https://artemis.apache.org/components/artemis/documentation/latest/libaio.html#runtime-dependencies).
...
|     |     |
| --- | --- |
|  | Using the file-based journal is the **recommended** configuration as it typically offers the highest levels of performance.<br>Performance for both paging and large messages is especially diminished with JDBC.<br>The JDBC persistence layer is targeted to those users who _must_ use a database e.g. due to internal company policy. |
...
1. See the documentation on [adding runtime dependencies](https://artemis.apache.org/components/artemis/documentation/latest/using-server.html#adding-runtime-dependencies) to understand how to make the JDBC driver available to the broker.
...
|     |     |
| --- | --- |
|  | When configuring the server using the XML configuration files please ensure to escape any illegal chars;<br>"&" for example, is typical in JDBC connection url and should be escaped to "&". |
...
|     |     |
| --- | --- |
|  | Some DBMS (e.g. Oracle, 30 chars) have restrictions on the size of table names.<br>This should be taken into consideration when configuring table names for the database store.<br>Pay particular attention to the page-store table name, which can be appended with a unique ID of up to 20 characters.<br>(for Oracle this would mean configuring a page-store-table-name of max size of 10 chars). |
...
You can find the documentation of the data source properties at [https://commons.apache.org/proper/commons-dbcp/configuration.html](https://commons.apache.org/proper/commons-dbcp/configuration.html).

To mask the value of a property you can use the same procedure used to [mask passwords](https://artemis.apache.org/components/artemis/documentation/latest/masking-passwords.html#masking-passwords).
