[Skip to main content](https://www.rabbitmq.com/docs/reliability#__docusaurus_skipToContent_fallback)
...
[4.3](https://www.rabbitmq.com/docs/reliability)

- **Release series**
- [Next](https://www.rabbitmq.com/docs/next/reliability)
- [4.3](https://www.rabbitmq.com/docs/reliability)
- [4.2](https://www.rabbitmq.com/docs/4.2/reliability)
- [4.1](https://www.rabbitmq.com/docs/4.1/reliability)
- [4.0](https://www.rabbitmq.com/docs/4.0/reliability)
- [3.13](https://www.rabbitmq.com/docs/3.13/reliability)
...
## Overview [​](https://www.rabbitmq.com/docs/reliability\#overview "Direct link to Overview")

This guides provides an overview features of
RabbitMQ and (some) of its supported protocols related to data safety
and failure handling.

They help application developers and operators achieve **reliable delivery**,
that is, to ensure that messages are always delivered, even encountering failures
of various kinds.

Data safety is a joint responsibility of RabbitMQ nodes, [publishers](https://www.rabbitmq.com/docs/publishers)
and [consumers](https://www.rabbitmq.com/docs/consumers). Therefore, this guide provides an overview of
...
- [Acknowledgements and Confirms](https://www.rabbitmq.com/docs/confirms)
- [Clustering](https://www.rabbitmq.com/docs/clustering)
- [Replicated Queues](https://www.rabbitmq.com/docs/quorum-queues) and [Streams](https://www.rabbitmq.com/docs/streams)
...
- [Monitoring, Metrics and Health Checks](https://www.rabbitmq.com/docs/monitoring)

## What Can Fail? [​](https://www.rabbitmq.com/docs/reliability\#what-can-fail "Direct link to What Can Fail?")
...
Not only can networks fail, [firewalls can interrupt connections](https://www.rabbitmq.com/docs/heartbeats#tcp-proxies)
they consider to be idle, and network failures [take time to detect](https://www.rabbitmq.com/docs/heartbeats).
...
logic errors can cause [channel](https://www.rabbitmq.com/docs/channels#error-handling) or [connection errors](https://www.rabbitmq.com/docs/connections#error-handling) which force the
...
and so on. Those failures can be detected with [monitoring, metrics and health checks](https://www.rabbitmq.com/docs/reliability#monitoring).

## Connection Failures [​](https://www.rabbitmq.com/docs/reliability\#connection-failures "Direct link to Connection Failures")
...
event handlers. See client documentation, such as the [Java](https://www.rabbitmq.com/client-libraries/java-api-guide)
and [.NET client guides](https://www.rabbitmq.com/client-libraries/dotnet-api-guide), to learn more.

### Acknowledgements and Confirms [​](https://www.rabbitmq.com/docs/reliability\#confirms "Direct link to Acknowledgements and Confirms")
...
need to be retransmitted. [Acknowledgements](https://www.rabbitmq.com/docs/confirms) let the server and
...
responsibility for them. The details are covered in the [Acknowledgements and Confirms guide](https://www.rabbitmq.com/docs/confirms).
...
## Detecting Dead TCP Connections with Heartbeats [​](https://www.rabbitmq.com/docs/reliability\#heartbeats "Direct link to Detecting Dead TCP Connections with Heartbeats")
...
[heartbeat feature](https://www.rabbitmq.com/docs/heartbeats) to ensure that the application layer
...
connections. See the [guide on heartbeats](https://www.rabbitmq.com/docs/heartbeats) for details.

## Data Safety on the RabbitMQ Side [​](https://www.rabbitmq.com/docs/reliability\#broker-side "Direct link to Data Safety on the RabbitMQ Side")
...
With some messaging protocols supported by RabbitMQ, applications control [durability of queues and messages](https://www.rabbitmq.com/docs/queues#durability).
It's therefore critically important that durable queues (or replicated queue types covered below)
are used for important data, and messages are [published as persistent by publishers](https://www.rabbitmq.com/docs/publishers#message-properties).

## Clustering and Queue Content Replication [​](https://www.rabbitmq.com/docs/reliability\#clustering "Direct link to Clustering and Queue Content Replication")

[Clusters of nodes](https://www.rabbitmq.com/docs/clustering) offer redundancy and can tolerate failure of a single node.
In a RabbitMQ cluster, all definitions (of exchanges, bindings, users, etc) are replicated across the entire
cluster.

[Quorum queues](https://www.rabbitmq.com/docs/quorum-queues), [streams](https://www.rabbitmq.com/docs/streams) and superstreams (partitioned streams) are
...
[Exclusive queues](https://www.rabbitmq.com/docs/queues#exclusive-queues) are tied to the lifecycle of their connection and thus are never
...
## Data Safety on the Publisher Side [​](https://www.rabbitmq.com/docs/reliability\#publisher-side "Direct link to Data Safety on the Publisher Side")
...
### Ensuring that Messages are Routed [​](https://www.rabbitmq.com/docs/reliability\#routing "Direct link to Ensuring that Messages are Routed")
...
appropriately bound. See the [Publishers guide](https://www.rabbitmq.com/docs/publishers) for details.
...
and the queue leader replica. See [inter-node heartbeat guide](https://www.rabbitmq.com/docs/nettick) for
more details.

## Data Safety on the Consumer Side [​](https://www.rabbitmq.com/docs/reliability\#consumer-side "Direct link to Data Safety on the Consumer Side")

In the event of network failure (or a node failure), messages can be
[redelivered](https://www.rabbitmq.com/docs/consumers#message-properties), and consumers must be prepared to handle
...
If a message is delivered to a consumer and then requeued, either [automatically](https://www.rabbitmq.com/docs/confirms#automatic-requeueing) by
...
### Unprocessable Deliveries [​](https://www.rabbitmq.com/docs/reliability\#unprocessable-deliveries "Direct link to Unprocessable Deliveries")
...
to [dead-letter](https://www.rabbitmq.com/docs/dlx) it instead).

### Consumer Cancel Notification [​](https://www.rabbitmq.com/docs/reliability\#cancel-notification "Direct link to Consumer Cancel Notification")

When the queue a consumer was consuming from has been deleted, RabbitMQ will [notify the consumer](https://www.rabbitmq.com/docs/consumer-cancel).
...
## Federation and Shovel [​](https://www.rabbitmq.com/docs/reliability\#federation-and-shovel "Direct link to Federation and Shovel")

RabbitMQ provides two plugins to assist with distributing nodes over
unreliable networks (such as wide-area networks): [Federation](https://www.rabbitmq.com/docs/federation) and
the [Shovel](https://www.rabbitmq.com/docs/shovel). Both will recover from network failures and retransmit messages when necessary.
...
from node failures, including permanent ( **fail-stop**) scenarios.
...
## Monitoring and Health Checks [​](https://www.rabbitmq.com/docs/reliability\#monitoring "Direct link to Monitoring and Health Checks")

Some failure scenarios are subtle and hard to observe or detect. For example, a slow [connection leak](https://www.rabbitmq.com/docs/connections)
can build up over time and like a chronic disease, go unnoticed for a period of time. [Monitoring and metrics](https://www.rabbitmq.com/docs/monitoring)
is the way to detect many types of failures. Longer-term metric data collected using tools such as [Prometheus](https://www.rabbitmq.com/docs/prometheus)
can help spot irregularities and problematic patterns in system behaviour.

In addition to monitoring, [health checks](https://www.rabbitmq.com/docs/monitoring#health-checks) is another tool that can be used to detect
**point-in-time** problems, that is, problems observable at the moment. Extensive health check coverage can result in false positives so more checks is not always better.

Both monitoring and health checks are covered in a [dedicated guide](https://www.rabbitmq.com/docs/monitoring).

[Edit this page](https://github.com/rabbitmq/rabbitmq-website/tree/main/versioned_docs/version-4.3/reliability.md)

[Previous\\
\\
Channels](https://www.rabbitmq.com/docs/channels) [Next\\
\\
Consumer Acknowledgements and Publisher Confirms](https://www.rabbitmq.com/docs/confirms)

- [Overview](https://www.rabbitmq.com/docs/reliability#overview)
- [What Can Fail?](https://www.rabbitmq.com/docs/reliability#what-can-fail)
- [Connection Failures](https://www.rabbitmq.com/docs/reliability#connection-failures)
  - [Acknowledgements and Confirms](https://www.rabbitmq.com/docs/reliability#confirms)
- [Detecting Dead TCP Connections with Heartbeats](https://www.rabbitmq.com/docs/reliability#heartbeats)
- [Data Safety on the RabbitMQ Side](https://www.rabbitmq.com/docs/reliability#broker-side)
- [Clustering and Queue Content Replication](https://www.rabbitmq.com/docs/reliability#clustering)
- [Data Safety on the Publisher Side](https://www.rabbitmq.com/docs/reliability#publisher-side)
  - [Ensuring that Messages are Routed](https://www.rabbitmq.com/docs/reliability#routing)
- [Data Safety on the Consumer Side](https://www.rabbitmq.com/docs/reliability#consumer-side)
  - [Unprocessable Deliveries](https://www.rabbitmq.com/docs/reliability#unprocessable-deliveries)
  - [Consumer Cancel Notification](https://www.rabbitmq.com/docs/reliability#cancel-notification)
- [Federation and Shovel](https://www.rabbitmq.com/docs/reliability#federation-and-shovel)
- [Monitoring and Health Checks](https://www.rabbitmq.com/docs/reliability#monitoring)
