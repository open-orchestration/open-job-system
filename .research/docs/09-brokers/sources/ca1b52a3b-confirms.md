[Skip to main content](https://www.rabbitmq.com/docs/confirms#__docusaurus_skipToContent_fallback)
...
[4.3](https://www.rabbitmq.com/docs/confirms)

- **Release series**
- [Next](https://www.rabbitmq.com/docs/next/confirms)
- [4.3](https://www.rabbitmq.com/docs/confirms)
- [4.2](https://www.rabbitmq.com/docs/4.2/confirms)
- [4.1](https://www.rabbitmq.com/docs/4.1/confirms)
- [4.0](https://www.rabbitmq.com/docs/4.0/confirms)
- [3.13](https://www.rabbitmq.com/docs/3.13/confirms)
...
## Overview [​](https://www.rabbitmq.com/docs/confirms\#overview "Direct link to Overview")

This guide covers two related features related to [data safety](https://www.rabbitmq.com/docs/reliability), consumer Acknowledgements
and publisher confirms:

- [Why acknowledgements exist](https://www.rabbitmq.com/docs/confirms#basics)
- [Manual and automatic](https://www.rabbitmq.com/docs/confirms#acknowledgement-modes) acknowledgement modes
- [Acknowledgement API](https://www.rabbitmq.com/docs/confirms#consumer-acks-api-elements), including [multi-acks](https://www.rabbitmq.com/docs/confirms#consumer-acks-multiple-parameter) and [requeueing](https://www.rabbitmq.com/docs/confirms#consumer-nacks-requeue)
- [Automatic requeueing](https://www.rabbitmq.com/docs/confirms#automatic-requeueing) on connection loss or channel closure
- [Channel prefetch](https://www.rabbitmq.com/docs/confirms#channel-qos-prefetch) and its [effects on throughput](https://www.rabbitmq.com/docs/confirms#channel-qos-prefetch-throughput)
- Most common [client errors](https://www.rabbitmq.com/docs/confirms#consumer-acks-double-acking)
- [Publisher confirms](https://www.rabbitmq.com/docs/confirms#publisher-confirms) and related publisher data safety topics
...
More related topics are covered in the [Publisher](https://www.rabbitmq.com/docs/publishers) and [Consumer](https://www.rabbitmq.com/docs/consumers) guides.

## The Basics [​](https://www.rabbitmq.com/docs/confirms\#basics "Direct link to The Basics")
...
Delivery processing acknowledgements from [consumers](https://www.rabbitmq.com/docs/consumers) to RabbitMQ
are known as acknowledgements in messaging protocols; broker
acknowledgements to [publishers](https://www.rabbitmq.com/docs/publishers) are a protocol extension called
...
they are **essential for data safety**, for which applications are
responsible as much as RabbitMQ nodes are.

### Are Publisher Confirms Related to Consumer Delivery Acknowledgements? [​](https://www.rabbitmq.com/docs/confirms\#relation "Direct link to Are Publisher Confirms Related to Consumer Delivery Acknowledgements?")

[Publisher confirms](https://www.rabbitmq.com/docs/confirms#publisher-confirms) and [consumer delivery acknowledgements](https://www.rabbitmq.com/docs/confirms#consumer-acknowledgements)
...
with node it is connected to, and the queue (or [stream](https://www.rabbitmq.com/docs/streams)) leader replica.
...
that need an explicit acknowledgement from the peer. [RabbitMQ tutorial #6](https://www.rabbitmq.com/tutorials)
demonstrates the basics of how that's done, and [Direct Reply-to](https://www.rabbitmq.com/docs/direct-reply-to) provides
...
## (Consumer) Delivery Acknowledgements [​](https://www.rabbitmq.com/docs/confirms\#consumer-acknowledgements "Direct link to (Consumer) Delivery Acknowledgements")
...
also covered in [RabbitMQ tutorial #2](https://www.rabbitmq.com/tutorials).

### Delivery Identifiers: Delivery Tags [​](https://www.rabbitmq.com/docs/confirms\#consumer-acks-delivery-tags "Direct link to Delivery Identifiers: Delivery Tags")
...
### Consumer Acknowledgement Modes and Data Safety Considerations [​](https://www.rabbitmq.com/docs/confirms\#acknowledgement-modes "Direct link to Consumer Acknowledgement Modes and Data Safety Considerations")
...
- `basic.nack` is used for negative acknowledgements (note: this is a [RabbitMQ extension to AMQP 0-9-1](https://www.rabbitmq.com/docs/nack))
...
Therefore, automatic message acknowledgement **should be considered unsafe**
...
### Positively Acknowledging Deliveries [​](https://www.rabbitmq.com/docs/confirms\#consumer-acks-api-elements "Direct link to Positively Acknowledging Deliveries")
...
### Acknowledging Multiple Deliveries at Once [​](https://www.rabbitmq.com/docs/confirms\#consumer-acks-multiple-parameter "Direct link to Acknowledging Multiple Deliveries at Once")
...
### Negative Acknowledgement and Requeuing of Deliveries [​](https://www.rabbitmq.com/docs/confirms\#consumer-nacks-requeue "Direct link to Negative Acknowledgement and Requeuing of Deliveries")
...
Alternatively, when this field is set to `false`, the message will be routed to a [Dead Letter Exchange](https://www.rabbitmq.com/docs/dlx) if it
...
### Channel Prefetch Setting (QoS) [​](https://www.rabbitmq.com/docs/confirms\#channel-qos-prefetch "Direct link to Channel Prefetch Setting (QoS)")
...
Acknowledging [multiple messages at once](https://www.rabbitmq.com/docs/confirms#consumer-acks-multiple-parameter)
...
#### "Unlimited" Prefetch [​](https://www.rabbitmq.com/docs/confirms\#unlimited-prefetch "Direct link to \"Unlimited\" Prefetch")
...
Message sources, such as [queues](https://www.rabbitmq.com/docs/queues) and [streams](https://www.rabbitmq.com/docs/streams), can still introduce
...
For example, [quorum queues](https://www.rabbitmq.com/docs/quorum-queues) cap consumer prefetch maximum
to 2,000 to limit a potentially runaway Raft log growth, similarly to how quorum queues
[enforce delivery acknowledgement timeouts](https://www.rabbitmq.com/docs/consumers#acknowledgement-timeout) on consumers.

As [Consumer Acknowledgement Modes, Prefetch and Throughput](https://www.rabbitmq.com/docs/confirms#channel-qos-prefetch-throughput) below explains,
a prefetch of 2,000 won't be meaningfully different from that of 300 in terms of consumer throughput.

In scenarios where maximum throughput is the highest priority, use [streams and partitioned streams](https://www.rabbitmq.com/docs/streams)
with a RabbitMQ Stream Protocol client instead of increasing channel prefetch.

#### Per-channel, Per-consumer and Global Prefetch [​](https://www.rabbitmq.com/docs/confirms\#per-channel-per-consumer-and-global-prefetch "Direct link to Per-channel, Per-consumer and Global Prefetch")

The QoS setting can be configured for a specific channel or a specific consumer.
The [Consumer Prefetch](https://www.rabbitmq.com/docs/consumer-prefetch) guide explains
the effects of this scoping.

#### Prefetch and Polling Consumers [​](https://www.rabbitmq.com/docs/confirms\#prefetch-and-polling-consumers "Direct link to Prefetch and Polling Consumers")

The QoS prefetch setting has no effect on messages fetched using the `basic.get`
("pull API"), even in manual confirmation mode.

### Consumer Acknowledgement Modes, Prefetch and Throughput [​](https://www.rabbitmq.com/docs/confirms\#channel-qos-prefetch-throughput "Direct link to Consumer Acknowledgement Modes, Prefetch and Throughput")
...
### When Consumers Fail or Lose Connection: Automatic Requeueing [​](https://www.rabbitmq.com/docs/confirms\#automatic-requeueing "Direct link to When Consumers Fail or Lose Connection: Automatic Requeueing")
...
Note that it takes a period of time to [detect an unavailable client](https://www.rabbitmq.com/docs/heartbeats).
...
### Client Errors: Double Acking and Unknown Tags [​](https://www.rabbitmq.com/docs/confirms\#consumer-acks-double-acking "Direct link to Client Errors: Double Acking and Unknown Tags")
...
## Publisher Confirms [​](https://www.rabbitmq.com/docs/confirms\#publisher-confirms "Direct link to Publisher Confirms")

Networks can fail in less-than-obvious ways and detecting some failures [takes time](https://www.rabbitmq.com/docs/heartbeats).
...
### Negative Acknowledgments for Publishes [​](https://www.rabbitmq.com/docs/confirms\#server-sent-nacks "Direct link to Negative Acknowledgments for Publishes")
...
### When Will Published Messages Be Confirmed by the Broker? [​](https://www.rabbitmq.com/docs/confirms\#when-publishes-are-confirmed "Direct link to When Will Published Messages Be Confirmed by the Broker?")
...
messages routed to durable queues, this **means persisting**
**to disk**. For [quorum queues](https://www.rabbitmq.com/docs/quorum-queues),
this means that a quorum replicas have accepted and confirmed
the message to the elected leader.

### Ack Latency for Persistent Messages [​](https://www.rabbitmq.com/docs/confirms\#publisher-confirms-latency "Direct link to Ack Latency for Persistent Messages")
...
### Ordering Considerations for Publisher Confirms [​](https://www.rabbitmq.com/docs/confirms\#publisher-confirms-ordering "Direct link to Ordering Considerations for Publisher Confirms")
...
### Publisher Confirms and Guaranteed Delivery [​](https://www.rabbitmq.com/docs/confirms\#publisher-confirms-and-guaranteed-delivery "Direct link to Publisher Confirms and Guaranteed Delivery")
...
## Limitations [​](https://www.rabbitmq.com/docs/confirms\#limitations "Direct link to Limitations")

### Maximum Delivery Tag [​](https://www.rabbitmq.com/docs/confirms\#publisher-confirms-and-guaranteed-delivery "Direct link to Maximum Delivery Tag")
...
[Edit this page](https://github.com/rabbitmq/rabbitmq-website/tree/main/versioned_docs/version-4.3/confirms.md)

[Previous\\
\\
Reliability and Data Safety](https://www.rabbitmq.com/docs/reliability) [Next\\
\\
Network Distribution](https://www.rabbitmq.com/docs/distributed)

- [Overview](https://www.rabbitmq.com/docs/confirms#overview)
- [The Basics](https://www.rabbitmq.com/docs/confirms#basics)
  - [Are Publisher Confirms Related to Consumer Delivery Acknowledgements?](https://www.rabbitmq.com/docs/confirms#relation)
- [(Consumer) Delivery Acknowledgements](https://www.rabbitmq.com/docs/confirms#consumer-acknowledgements)
  - [Delivery Identifiers: Delivery Tags](https://www.rabbitmq.com/docs/confirms#consumer-acks-delivery-tags)
  - [Consumer Acknowledgement Modes and Data Safety Considerations](https://www.rabbitmq.com/docs/confirms#acknowledgement-modes)
  - [Positively Acknowledging Deliveries](https://www.rabbitmq.com/docs/confirms#consumer-acks-api-elements)
  - [Acknowledging Multiple Deliveries at Once](https://www.rabbitmq.com/docs/confirms#consumer-acks-multiple-parameter)
  ...
  - [Consumer Acknowledgement Modes, Prefetch and Throughput](https://www.rabbitmq.com/docs/confirms#channel-qos-prefetch-throughput)
  - [When Consumers Fail or Lose Connection: Automatic Requeueing](https://www.rabbitmq.com/docs/confirms#automatic-requeueing)
  - [Client Errors: Double Acking and Unknown Tags](https://www.rabbitmq.com/docs/confirms#consumer-acks-double-acking)
- [Publisher Confirms](https://www.rabbitmq.com/docs/confirms#publisher-confirms)
  - [Negative Acknowledgments for Publishes](https://www.rabbitmq.com/docs/confirms#server-sent-nacks)
  - [When Will Published Messages Be Confirmed by the Broker?](https://www.rabbitmq.com/docs/confirms#when-publishes-are-confirmed)
  - [Ack Latency for Persistent Messages](https://www.rabbitmq.com/docs/confirms#publisher-confirms-latency)
  - [Ordering Considerations for Publisher Confirms](https://www.rabbitmq.com/docs/confirms#publisher-confirms-ordering)
  - [Publisher Confirms and Guaranteed Delivery](https://www.rabbitmq.com/docs/confirms#publisher-confirms-and-guaranteed-delivery)
- [Limitations](https://www.rabbitmq.com/docs/confirms#limitations)
  - [Maximum Delivery Tag](https://www.rabbitmq.com/docs/confirms#publisher-confirms-and-guaranteed-delivery)
