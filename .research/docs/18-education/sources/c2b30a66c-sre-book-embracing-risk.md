...
Consider Bigtable [\[Cha06\]](https://sre.google/sre-book/bibliography#Cha06), a massive-scale distributed storage system for structured data. Some consumer services serve data directly from Bigtable in the path of a user request. Such services need low latency and high reliability. Other teams use Bigtable as a repository for data that they use to perform offline analysis (e.g., MapReduce) on a regular basis. These teams tend to be more concerned about throughput than reliability. Risk tolerance for these two use cases is quite distinct.
...
# Motivation for Error Budgets[14](https://sre.google/sre-book/embracing-risk/\#id-na2u1S2SKi1)
...
Software fault toleranceHow hardened do we make the software to unexpected events? Too little, and we have a brittle, unusable product. Too much, and we have a product no one wants to use (but that runs very stably).TestingAgain, not enough testing and you have embarrassing outages, privacy data leaks, or a number of other press-worthy events. Too much testing, and you might lose your market.Push frequencyEvery push is risky. How much should we work on reducing that risk, versus doing other work?Canary duration and sizeIt’s a best practice to test a new release on some small subset of a typical workload, a practice often called _canarying_. How long do we wait, and how big is the canary?
...
[14](https://sre.google/sre-book/embracing-risk/#id-na2u1S2SKi1-marker)An early version of this section appeared as an article in _;login:_ (August 2015, vol. 40, no. 4).

[15](https://sre.google/sre-book/embracing-risk/#id-bb2umSeIPu8iV-marker)Known as "bang/bang" control—see [_https://en.wikipedia.org/wiki/Bang–bang\_control_](https://en.wikipedia.org/wiki/Bang%E2%80%93bang_control).
