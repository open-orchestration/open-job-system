...
> operational complexity"* and expanded into a full survey as all 17 catalog topics were
> re-researched through the Open Research System pipeline. **19 findings promoted** (each
> passed citation, faithfulness, and independent-reviewer gates); certainty is **moderate**
> except where noted.
...
- The original five-way comparison (BullMQ/Sidekiq/Celery/**Temporal**/**River** on
  throughput & operational complexity) is **not fully answered**: Temporal and River were
  abstained in the seed run, and the `throughput`/`operational-complexity` *dimensions* were
  not separately sourced — the survey covers the field broadly rather than scoring that matrix.
