...
- About
  ...
  - [Release Notes](https://slurm.schedmd.com/release_notes.html)
- Using
  - [Documentation](https://slurm.schedmd.com/documentation.html)
  - [FAQ](https://slurm.schedmd.com/faq.html)
  - [Publications](https://slurm.schedmd.com/publications.html)
- Installing
...
- Getting Help
  - [Mailing Lists](https://slurm.schedmd.com/mail.html)
...
In the example below we find there are two partitions: _debug_
and _batch_.
The _\*_ following the name _debug_ indicates this is the
default partition for submitted jobs.
We see that both partitions are in an _UP_ state.
Some configurations may include partitions for larger jobs
that are _DOWN_ except on weekends or at night. The information
about each partition may be split over more than one line so that
nodes in different states can be identified.
In this case, the two nodes _adev\[1-2\]_ are _down_.
The _\*_ following the state _down_ indicate the nodes are
not responding. Note the use of a concise expression for node
name specification with a common prefix _adev_ and numeric
...
squeue command. The
_ST_ field is job state.
Two jobs are in a running state ( _R_ is an abbreviation
for _Running_) while one job is in a pending state
( _PD_ is an abbreviation for _Pending_).
The _TIME_ field shows how long the jobs have run
for using the format _days-hours:minutes:seconds_.
The _NODELIST(REASON)_ field indicates where the
job is running or the reason it is still pending. Typical
reasons for pending jobs are _Resources_ (waiting
for resources to become available) and _Priority_
...
on three nodes ( _-N3_) and include task numbers on the output ( _-l_).
...
/bin/hostname in four tasks ( _-n4_).
...
In this example the script name is _my.script_ and we explicitly use
the nodes adev9 and adev10 ( _-w "adev\[9-10\]"_, note the use of a
...
differ, see [MPI](https://slurm.schedmd.com/quickstart.html#mpi) details below).
Finally the shell created by salloc would
be terminated using the _exit_ command.
...
the executable program _a.out_ to _/tmp/joe.a.out_ on local storage
...
[Job arrays](https://slurm.schedmd.com/job_array.html) are an efficient mechanism of
