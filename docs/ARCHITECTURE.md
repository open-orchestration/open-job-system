...
The decision log moved to `DECISIONS.md`, the normative source of truth for architectural decisions. It holds every record (ADR-001…011) in full MADR format — context, decision drivers, considered options, the chosen outcome with rationale, per-option pros/cons, and how conformance is confirmed — plus the **How We Decide** process (when to write a record, the status lifecycle, numbering, and superseding). The inline `(ADR-NNN)` tags throughout this document resolve to records there. On any conflict between this document's prose and the decision log, the decision log wins.
