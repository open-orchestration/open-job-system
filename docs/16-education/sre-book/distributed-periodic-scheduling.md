...
Written by Štěpán Davidovič[114](https://sre.google/sre-book/distributed-periodic-scheduling/#id-WakuESLFd)
...
conjunction with a datacenter scheduling system like Borg [\[Ver15\]](https://sre.google/sre-book/bibliography#Ver15).
...
We deploy multiple replicas of the cron service and use the [Paxos distributed consensus algorithm](https://sre.google/sre-book/managing-critical-state/) (see [Managing Critical State: Distributed Consensus for Reliability](https://sre.google/sre-book/managing-critical-state/)) to ensure they have consistent state. As long as the majority of group members are available, the distributed system as a whole can successfully process new state changes despite the failure of bounded subsets of the infrastructure.

As shown in [Figure 24-1](https://sre.google/sre-book/distributed-periodic-scheduling/#fig_reliable_cron_interactions), the distributed cron uses a single leader job, which is the only replica that can modify the shared state, as well as the only replica that can launch cron jobs. We take advantage of the fact that the variant of Paxos we use, Fast Paxos [\[Lam06\]](https://sre.google/sre-book/bibliography#Lam06), uses a leader replica internally as an optimization—the Fast Paxos leader replica also acts as the cron service leader.

![The interactions between distributed cron replicas.](https://lh3.googleusercontent.com/IdmUKKtm5nsPO1TpsDA3OFpFBaJYYUMiYQB12iuCIL2cvQXCdZJgDsKWNOdGRqNST2gzZBYXA82rYG8863OGypXATmZ_x9T12Ntd=s900)Figure 24-1. The interactions between distributed cron replicas
...
Upon reaching the scheduled launch time, the leader replica announces that it is about to start this particular cron job's launch, and calculates the new scheduled launch time, just like a regular `crond` implementation would. Of course, as with regular `crond`, a cron job launch specification may have changed since the last execution, and this launch specification must be kept in sync with the followers as well. Simply identifying the cron job is not enough: we should also uniquely identify the particular launch using the start time; otherwise, ambiguity in cron job launch tracking may occur. (Such ambiguity is especially likely in the case of high-frequency cron jobs, such as those running every minute.) As seen in [Figure 24-2](https://sre.google/sre-book/distributed-periodic-scheduling/#fig_reliable_cron_job_launch), this communication is performed over Paxos.
...
![Illustration of progress of a cron job launch, from the leader's perspective.](https://lh3.googleusercontent.com/VLNsVzQoxeEQbEIBEr5zkgbQhJpzDdo5mWAOY_BSZBvRdpcHSBgeA3JFSQH0l0aFHzUu4Z-jTV2wDbR7WcMgqZo5XVufHTvolTNabQ=s900)Figure 24-2. Illustration of progress of a cron job launch, from the leader's perspective
...
Despite this change, the load caused by the cron jobs is still very spiky. The graph in [Figure 24-3](https://sre.google/sre-book/distributed-periodic-scheduling/#fig_reliable_cron_job_launches) illustrates the aggregate global number of launches of cron jobs at Google. This
...
![The number of cron jobs launched globally.](https://lh3.googleusercontent.com/w-8lRqMQOpTNMEjxqL4DmAXcRi-inPrvQWtjTPWAEsNWStrbgdbIH80Wt8eF_I-vMgp8nrPEBCRovuXZOGgnjwIEYR9utemEGgZ5=s900)Figure 24-3. The number of cron jobs launched globally
...
[114](https://sre.google/sre-book/distributed-periodic-scheduling/#id-WakuESLFd-marker)This chapter was previously published in part in _ACM Queue_ (March 2015, vol. 13, issue 3).

[115](https://sre.google/sre-book/distributed-periodic-scheduling/#id-z0AIxSrSnSWIVtOTJ-marker)Failure of individual jobs is beyond the scope of this analysis.
